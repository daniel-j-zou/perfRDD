"""Re-render α(η) + h-components (with η density overlay) and the utility
curve U(φ) for the four 'linear ≈ q_nonlinear' datasets (gpa, nhanes, oulad,
lending_club), then compose a 4-row figure with stats sidebar.

Stats sidebar includes the X / Y / Q definitions, treatment rule, both linear
and q_nonlinear first-stage R², avg α, etc. Histogram is drawn on a twinx axis
behind the curves — tweak ALPHA_FILL to control opacity.
"""
...
RUNS = ROOT / "runs"
OUT_DIR = RUNS / "four_datasets_linear_alpha_eta"
OUT_DIR.mkdir(parents=True, exist_ok=True)

ALPHA_FILL = 0.32
BINS = 70

# (name, display title, Q, Y, X (short), treatment rule)
DATASETS = [
    (
        "gpa",
...
def plot_alpha_with_eta(fit, D, name: str, out_path: Path) -> float:
    ...
    alpha = Phi_eval @ fit.omega_treat
    h_treat_eval = Phi_eval @ (fit.omega_base + fit.omega_treat)

    grid_full = np.linspace(fit.info["lo"], fit.info["hi"], 500)
    h_base_full = _eval_basis(grid_full, fit.info) @ fit.omega_base
    ...
    ax[0].plot(grid_eval, alpha, "C0-", lw=2.2,
    ...
    # --- right: h components + density underlay ---
    ...
    ax[1].plot(grid_full, h_base_full, "C0-", lw=1.8,
               label="control component", zorder=4)
    ax[1].plot(grid_eval, h_treat_eval, "C3-", lw=1.8,
               label="treated component", zorder=4)
...
def plot_utility_for_fit(fit, D, Q, X, threshold: float, name: str,
                         out_path: Path) -> dict:
    # Scale cost grid by support-restricted avg α (same logic as perfrdd()).
    ...
    phi_stars = _plot_utility(phi_grid, utils, threshold, out_path, name)
    return phi_stars
...
def main() -> None:
    ...
    for name, *_ in DATASETS:
        ...
        avg = plot_alpha_with_eta(fit, D, name, a_path)
        phi_stars = plot_utility_for_fit(fit, D, Q, X, threshold, name, u_path)
        fits[name] = {"avg_alpha": avg, "phi_star": phi_stars}
    ...
    fig.suptitle(
        "Performative RDD — Linear setting, 4 datasets: α(η) + h-components "
        "(η density overlay) and U(φ)",
    ...
    comp = RUNS / "four_datasets_linear_alpha_eta_utility_compare.png"

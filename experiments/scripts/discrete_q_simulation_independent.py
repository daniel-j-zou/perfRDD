"""Discrete-Q simulation, surrogate-detached DGP.

Companion to discrete_q_simulation.py. Identical structure, but the
surrogate Q_tilde is built from a NEW latent eta_tilde that is independent
of the latent eta driving Q and the Y-confounding. Formally:

    eta       ~ N(0, sigma_eta^2)    # drives the discrete score S and b(eta)
    eta_tilde ~ N(0, sigma_eta^2)    # drives the surrogate Q_tilde only
    eta_tilde indep eta

    Q_tilde = X_a^T delta_a + eta_tilde     (uses eta_tilde, not eta)
    S       = X_b^T delta_b + lambda*eta + u
    Q       = bin(S),  D = 1{Q > k_0}
    Y       = alpha_0 D + X_c^T beta_c + X_b^T beta_b + b(eta) + eps
              b(eta) = eta + 0.5 eta^2

The proposed estimator recovers eta_hat = Q_tilde - X_a^T delta_a_hat ~
eta_tilde, which is independent of eta. The partial-linear regression
Y ~ D + X + spline(eta_hat) then controls for the WRONG latent, so the
confounding from b(eta) survives in the residual and alpha_0 estimation
should be biased -- close to naive OLS.

Expected result: proposed and naive collapse onto the same bias; oracle
is unbiased; continuous-perfRDD targets a different (unrelated)
parameter.
"""
...
OUT_JSON = ROOT / "runs" / "discrete_q_simulation_independent.json"
OUT_FIG  = ROOT / "runs" / "discrete_q_simulation_independent.png"
...
def generate(n: int, seed: int, params=DGP) -> Dict[str, Any]:
    ...
    eta        = rng.normal(0.0, params['sigma_eta'], n)   # confounder of Y
    eta_tilde  = rng.normal(0.0, params['sigma_eta'], n)   # INDEPENDENT latent for the surrogate
    delta_a = params['delta_a_norm'] * np.ones(d_block) / np.sqrt(d_block)
    delta_b = params['delta_b_norm'] * np.ones(d_block) / np.sqrt(d_block)

    Q_tilde = X_a @ delta_a + eta_tilde         # surrogate uses eta_tilde, NOT eta

    u = rng.logistic(0.0, 1.0, n)
    S = X_b @ delta_b + params['lambda_eta_in_S'] * eta + u     # Q driven by true eta
    ...
    return dict(
        X=X, X_a=X_a, X_b=X_b, X_c=X_c, eta=eta, eta_tilde=eta_tilde,
...
def main():
    ...
    r2_QtQ = float(np.corrcoef(pilot['Q_tilde'], pilot['Q'].astype(float))[0, 1]**2)
    # Recovery diagnostic: how well does eta_hat from Q_tilde recover the TRUE eta vs eta_tilde?
    ...
    cor_etahat_eta       = float(np.corrcoef(eta_hat_pilot, pilot['eta'])[0, 1])
    cor_etahat_etatilde  = float(np.corrcoef(eta_hat_pilot, pilot['eta_tilde'])[0, 1])
    ...
    print(f"R^2(Q_tilde, Q):                         {r2_QtQ:.3f}")
    print(f"Cor(eta_hat, true eta):                  {cor_etahat_eta:+.3f}   <-- should be ~0")
    print(f"Cor(eta_hat, eta_tilde):                 {cor_etahat_etatilde:+.3f}   <-- should be high")
    ...
    for n in N_GRID:
        ...
        r_o = rmse(ora, true_alpha); r_p = rmse(pro, true_alpha); r_n = rmse(nai, true_alpha)
        print(f"  oracle:   med={np.nanmedian(ora):.3f}  RMSE={r_o:.3f}")
        print(f"  proposed: med={np.nanmedian(pro):.3f}  RMSE={r_p:.3f}  (vs naive: {r_p/r_n:.2f}x naive RMSE)")
        print(f"  naive:    med={np.nanmedian(nai):.3f}  RMSE={r_n:.3f}")
    ...
    OUT_JSON.write_text(json.dumps({
        ...
        'r2_Qtilde_Q': r2_QtQ,
        'cor_etahat_eta': cor_etahat_eta,
        'cor_etahat_etatilde': cor_etahat_etatilde,
        'results': results,
    ...
    estimators_for_alpha = [
        ...
        ('proposed', 'Proposed (η̂ from Q̃ ≠ η)',   'C0'),
        ('naive',    'Naive OLS',                  'C3'),
    ]
    estimators_for_cont = [
    ...
    # Panel 1: distribution of alpha_hat - alpha_0 (boxplots)
    ax = axes[0]
    ...
    fig.suptitle(
        r"Surrogate detached from $\eta$:  $\tilde Q = X_a^\top\delta_a + \tilde\eta$ with $\tilde\eta \perp\!\!\!\perp \eta$.  "
        fr"Cor($\hat\eta$, true $\eta$)$={cor_etahat_eta:+.2f}$,  Cor($\hat\eta$, $\tilde\eta$)$={cor_etahat_etatilde:+.2f}$",

"""Variant of four_alpha_b_decomp.py that extends α̂(η) + b̂(η) over the FULL
spline support, drawing the data-supported portion (treated eval window) as a
solid red line and the extrapolated portion (where ω_treat is pinned by the
ridge prior, not by data) as a dashed semi-transparent red line.

This makes visually explicit the claim that, outside the treated η window,
α̂(η) ≈ 0 and the sum collapses onto b̂(η).
"""
...
RUNS = ROOT / "runs"
OUT_DIR = RUNS / "four_datasets_linear_alpha_b_extended"
...
def plot_alpha_b_extended(fit, D, name: str, out_path: Path) -> float:
    eta_lo, eta_hi = fit.eta_eval
    eta_tr = fit.eta[D == 1]
    eta_co = fit.eta[D == 0]

    # Curves: alpha-only on eval window, b and (alpha+b) on FULL support.
    grid_eval = np.linspace(eta_lo, eta_hi, 500)
    Phi_eval = _eval_basis(grid_eval, fit.info)
    alpha_curve = Phi_eval @ fit.omega_treat

    grid_full = np.linspace(fit.info["lo"], fit.info["hi"], 600)
    Phi_full = _eval_basis(grid_full, fit.info)
    b_full = Phi_full @ fit.omega_base
    aplusb_full = Phi_full @ (fit.omega_base + fit.omega_treat)

    in_eval = (grid_full >= eta_lo) & (grid_full <= eta_hi)

    avg_alpha = float(alpha_curve.mean())

    fig, ax = plt.subplots(1, 2, figsize=(13, 4.8))

    # --- left panel: alpha(eta) + density underlay (unchanged) ---
    ...
    # --- right panel: b̂ (full) and α̂+b̂ (full, with extrapolation dashed) ---
    ...
    # b̂ — solid, full support
    ax[1].plot(grid_full, b_full, "C0-", lw=1.8,
               label=r"$\hat b(\eta)$", zorder=4)

    # α̂ + b̂ — solid red inside eval window, dashed lighter red outside
    inside = aplusb_full.copy()
    inside[~in_eval] = np.nan
    outside = aplusb_full.copy()
    outside[in_eval] = np.nan
    ax[1].plot(grid_full, inside, "C3-", lw=1.8,
               label=r"$\hat\alpha(\eta) + \hat b(\eta)$  (data-supported)",
               zorder=5)
    ax[1].plot(grid_full, outside, color="C3", lw=1.5, ls="--", alpha=0.55,
               label=r"$\hat\alpha(\eta) + \hat b(\eta)$  (ridge extrapolation)",
               zorder=5)

    ax[1].axvline(eta_lo, color="green", ls=":", alpha=0.6, zorder=3)
    ax[1].axvline(eta_hi, color="green", ls=":", alpha=0.6,
                  label="treated eval region", zorder=3)
    ax[1].set_xlabel(r"$\eta$")
    ax[1].set_ylabel("h")
    ax[1].set_title(
        r"$\hat b(\eta)$  and  $\hat\alpha(\eta) + \hat b(\eta)$  "
        r"(full support; dashed = ridge extrapolation)"
    )
...
def main() -> None:
    ...
    for name, *_ in DATASETS:
        ...
        avg = plot_alpha_b_extended(fit, D, name, a_path)
    ...
    fig.suptitle(
        r"Performative RDD — Linear setting, 4 datasets:  $\hat\alpha + \hat b$ "
        r"extended over full η support (dashed = ridge-anchored extrapolation)",
    ...
    comp = RUNS / "four_datasets_linear_alpha_b_extended_compare.png"

"""Four-dataset comparison of L2-ridge vs P-spline (second-difference) penalty
on ω_base.

For each dataset, six panels are rendered:
    [α(η)]  [b̂(η) and α̂(η)+b̂(η)]  [U(φ)]        ← L2 ridge (default)
    [α(η)]  [b̂(η) and α̂(η)+b̂(η)]  [U(φ)]        ← P-spline, λ = n

All six share a left-side stats sidebar with dataset definitions and headline
numbers under both methods.

The L2 fit reuses experiments.methods.perfrdd._fit_pooled_plm. The P-spline
fit is implemented inline, replacing the L2 penalty on ω_base with
λ · ‖D₂ ω_base‖² (with a tiny stabilising ε ridge). ω_treat keeps the original
L2 scaling so α stays comparable to the default model. The cost grid for U(φ)
is computed once from the L2 fit and shared between both methods so the
utility curves are directly comparable.
"""
...
RUNS = ROOT / "runs"
OUT = RUNS / "four_l2_vs_pspline_compare.png"
...
def fit_pspline(Q, X, Y, D, direction, lam_smooth):
    n = len(Y)
    n_tr = int(D.sum())
    ...
    return FitResult(
        gamma=gamma, eta=eta, omega_base=omega_base, omega_treat=omega_treat,
        beta=beta, info=info, eta_eval=(eval_lo, eval_hi),
        direction=direction, n=n, n_treated=n_tr,
    )


def overlay_density(ax, fit, D_arr, eta_range):
    """Draw treated/control η histograms on a twin y-axis behind the curves."""
...
def main() -> None:
    ...
    fig.suptitle(
        "Four-dataset comparison: L2 ridge vs P-spline curvature penalty on "
        r"$\omega_{\mathrm{base}}$",
        fontsize=15, fontweight="bold", y=0.975,
    )
    fig.text(0.31, 0.952,
             r"L2 ridge   ($\rho_b = \rho_t = 0.1 / \sqrt{n}$)",
             ha="center", fontsize=12, fontweight="bold", color="#222")
    fig.text(0.72, 0.952,
             r"P-spline on $\omega_{\mathrm{base}}$   "
             r"($\lambda = n$,  $\omega_{\mathrm{treat}}$ keeps L2)",
             ha="center", fontsize=12, fontweight="bold", color="#222")
    ...
    for i, (name, title, q_def, y_def, x_def, treat_def) in enumerate(DATASETS):
        ...
        # Fits.
        fit_l = _fit_pooled_plm(Q, X, Y, D, direction)
        fit_p = fit_pspline(Q, X, Y, D, direction, lam_smooth=LAM_SCALE * n_used)

        # Shared cost grid from the L2 fit so the two U(φ) panels are comparable.
        ...
        # --- L2 panels (cols 1, 2, 3) ---
        avg_l = plot_alpha(
            fig.add_subplot(gs[i, 1]), fit_l, D, f"{name}: α̂(η)  [L2]"
        )
        plot_decomp(
            fig.add_subplot(gs[i, 2]), fit_l, D, f"{name}: b̂ and α̂+b̂  [L2]"
        )
        phi_l = plot_utility(
        ...
        # --- P-spline panels (cols 4, 5, 6) ---
        avg_p = plot_alpha(
            fig.add_subplot(gs[i, 4]), fit_p, D, f"{name}: α̂(η)  [P-spline]"
        )
        plot_decomp(
            fig.add_subplot(gs[i, 5]), fit_p, D,
            f"{name}: b̂ and α̂+b̂  [P-spline]",
        )
        phi_p = plot_utility(
        ...
        # Append shifts to the sidebar.
        ax_txt.text(0.02, 0.20,
                    f"avg α (L2)       = {avg_l:.3g}\n"
                    f"avg α (P-spline) = {avg_p:.3g}\n"
        ...
        print(f"[{name}] n={n_used:,} n_treated={n_tr:,}  "
              f"avg α (L2)={avg_l:.3g}  avg α (P-spline)={avg_p:.3g}", flush=True)

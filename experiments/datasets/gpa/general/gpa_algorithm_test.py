...
def _fit_score_model(Q, X):
    ...
    A = np.column_stack([np.ones(n), X_matrix])

    # Solve for [intercept, gamma_1, gamma_2, ... gamma_k]
    sol, _, _, _ = np.linalg.lstsq(A, Q, rcond=None)

    gamma_intercept_hat = sol[0]
    gamma_slope_hat = sol[1:]  # This is now a vector of length k

    Q_hat = A @ sol
    eta_hat = Q - Q_hat

    return gamma_intercept_hat, gamma_slope_hat, Q_hat, eta_hat
...
def _fit_plm_theta(
    ...
    eta_hat: np.ndarray,
    matching: Matching,
) -> Tuple[float, np.ndarray]:  # Returns a vector for theta_hat
    t_idx = matching.admitted_idx
    s_idx = matching.denied_idx
    m = t_idx.shape[0]
    ...
    # Design matrix: [eta_t, X_diff]
    H = np.column_stack([eta_used, X_diff])

    coef, *_ = np.linalg.lstsq(H, Y_diff, rcond=None)
    alpha_hat = float(coef[0])
    theta_hat = coef[1:]  # Keep as a vector of all coefficients

    return alpha_hat, theta_hat

# def _estimate_G_from_projection(X, gamma_slope_hat):
#     X_matrix = np.asarray(X)
#     if X_matrix.ndim == 1:
#         # Fallback for scalar case
#         proj = gamma_slope_hat * X_matrix
#     else:
#         # Matrix multiplication for multivariate case: (n, k) @ (k,) -> (n,)
#         proj = X_matrix @ gamma_slope_hat
#
#     mu_proj = np.mean(proj)
#     std_proj = np.std(proj)
#
#     return NormalProjectionDistribution(mean=mu_proj, std=std_proj)

def _estimate_G_from_projection(
        gamma_slope_hat: np.ndarray,
        X: np.ndarray,
) -> NormalProjectionDistribution:
    """
    Estimate distribution of hat{gamma}^T X.
    """
    if X.ndim == 1:
        proj = gamma_slope_hat * X
    else:
        proj = X @ gamma_slope_hat
...
def _compute_residuals(Y: np.ndarray, X: np.ndarray, theta_hat: np.ndarray) -> np.ndarray:
    """
    R = Y - X @ theta_hat
    """
    # Use @ for matrix multiplication (n, k) @ (k,) -> (n,)
    if X.ndim == 1:
        return Y - theta_hat * X
    return Y - (X @ theta_hat)
...
all_cols = ['GPA_year2', 'dist_from_cut'] + x_cols
df_clean = df[all_cols].dropna()

dataset = Dataset(
    Y=df_clean['GPA_year2'].values.astype(float),
...
#Defining a function to run the necessary algorithm
def fit_global_model(dataset: Dataset):
    """
    Estimates alpha and theta using the entire dataset (Global OLS)
    instead of matched pairs.
    """
    Y = dataset.Y
    X = dataset.X
    Q = dataset.Q
    phi_0 = dataset.params.get('phi', 0.0)

    # 1. Create the treatment indicator: 1 if Q > phi_0
    T = (Q > phi_0).astype(float)

    # 2. Re-calculate eta_hat (the 'unexplained' part of the score)
    # This is necessary because alpha is the effect of the 'latent' term
    gamma_int, gamma_slope, _, eta_hat = _fit_score_model(Q, X)

    # 3. Design Matrix for Y: [1, T*eta_hat, X]
    # We use T*eta_hat because alpha in this model is the effect
    # of the treated latent component.
    intercept = np.ones(len(Y))
    H = np.column_stack([intercept, T * eta_hat, X])

    # 4. Solve OLS
    coef, _, _, _ = np.linalg.lstsq(H, Y, rcond=None)

    alpha_hat = float(coef[1])
    theta_hat = coef[2:]  # Vector of coefficients for your 8 variables

    return alpha_hat, theta_hat


alpha_hat, theta_hat = fit_global_model(dataset)

gamma_int, gamma_slope, Q_hat, eta_hat = _fit_score_model(dataset.Q, dataset.X)
G_dist = _estimate_G_from_projection(dataset.X, gamma_slope)


res_val = _compute_residuals(dataset.Y, dataset.X, theta_hat)
c = 0
mean_proj = G_dist.mean

def eval_u_global(phi_val):
    """Function to find the root of."""
    return alpha_hat * (phi_val - mean_proj) - c

# 4. Find the Optimal Cutoff (phi_hat)
phi_min, phi_max = -5.0, 5.0
bracket = _bracket_root_u_evo(
    phi_min,
    phi_max,
    eval_u_global,
...
if bracket:
    # Use the bracket to find the exact root
    phi_hat = brentq(eval_u_global, bracket[0], bracket[1])
else:
    # Fallback if the function never crosses zero in the range
    phi_hat = phi_min if eval_u_global(phi_min) < 0 else phi_max

# 5. Output Results
print(f"--- Global Estimation Results (No Matching) ---")
print(f"Global Alpha (Treatment Effect): {alpha_hat:.4f}")
print(f"Global Theta (Slopes): {theta_hat}")
print(f"Optimal Cutoff (centered): {phi_hat:.4f}")

# phi_hat = _bracket_root_u_evo(
#     alpha_hat=alpha_hat,
#     G_dist=G_dist,
#     residuals=res_val,
#     c=0,  # Your cost parameter
#     phi_min=-5, # Adjust based on your 'dist_from_cut' range
#     phi_max=5
# )
#
# print(f"Global Alpha: {alpha_hat:.4f}")
# print(f"Global Theta: {theta_hat}")
# print(f"Optimal Cutoff (phi_hat): {phi_hat:.4f}")
...
# )
#
# print(f"--- Estimation Results for {x_cols} ---")
# print(f"Estimated Alpha (Effect Intercept): {results.alpha_hat:.4f}")
# print(f"Estimated Theta (Covariate Slope):  {results.theta_hat}")
# print(f"Recommended Optimal Cutoff:        {results.phi_hat:.4f}")